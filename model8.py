...
from IPython.display import Audio, display  # For playing the audio in Jupyter
...
def identify_chord(notes):
    ...
    for perm in permutations(nums):
        if not perm:  # or continue if necessary
            continue
        root = perm[0]
        intervals = [(n - root) % 12 for n in perm]
        for chord_name, pattern in chords.items():
            if intervals == pattern:
                return f"{num_to_note[root]}{chord_name}"
    # Return the set of notes if no chord pattern is found
    return set(notes)
...
chord_progression = sound_to_chords(filename, 10)

# Create an interactive display
print("Starting chord detection and audio playback...")
for time, chord in chord_progression:
    if isinstance(chord, set):  # If chord is a set of notes
        print(f"Time: {time}s -> Notes: {chord}")
    else:
        print(f"Time: {time}s -> Chord: {chord}")

    # Play the audio and display chord in real-time
    display(Audio(filename))  # Play audio for each detected chord (or in real-time depending on your setup)
